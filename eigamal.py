...
(3149,7400),(8951,29526),(10536,6941),(1777,8737),
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<< eigamal function >>>>>>>>>>>>>>>>>>
def deeigamal(a, p, ciphertext):
    alpha_k = ciphertext[0]
    xBeta_k = ciphertext[1]
    Beta_k = pow(alpha_k, a, p)
    Beta_k_inv = extendEu(p,Beta_k)
    x = (xBeta_k*Beta_k_inv)%p
...
def decode(element):
    plain = ''
    char1 = element%26
    char2 = ((element - char1)/26)%26
    char3 = (element - char1 - char2*26)/26**2
    plain += chr(int(char3) + ord('a'))
    plain += chr(int(char2) + ord('a'))
    plain += chr(int(char1) + ord('a'))
...
for cipher in ciphers:
    digcode = deeigamal(a, p, cipher)
    plain = decode(digcode)
    print(plain, end = '')
